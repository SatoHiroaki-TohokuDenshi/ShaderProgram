...
Fbx::Fbx()
	...
	pMaterialList_(nullptr), pToon_(nullptr)
{
}
		...
		if (pTangent != nullptr) {
			for (int i = 0; i < 3; i++) {
				...
				vertices[index].tangent = {
					(float)tanVec[0],(float)tanVec[1], (float)tanVec[2], 0.0f
				};
			}
		}
		else {
...
void Fbx::DrawNMap(Transform& transform) {
	transform.Calclation();

	RenderNMap(transform);
}
...
void Fbx::DrawAlpha(Transform& transform) {
	transform.Calclation();

	Direct3D::SetShader(SHADER_ALPHA_B);
	Render3D(transform);
	Direct3D::SetShader(SHADER_ALPHA_F);
	Render3D(transform);
}

void Fbx::DrawWater(Transform& transform, float scroll) {
	transform.Calclation();

	RenderWater(transform, scroll);
}
...
void Fbx::RenderToon(Transform& transform) {
	...
	for (int i = 0; i < materialCount_; i++)
	{
		...
		ID3D11ShaderResourceView* pSRVToon = pToon_->GetSRV();
...
void Fbx::RenderEdge(Transform& transform) {
	Direct3D::SetShader(SHADER_TOON_EDGE);
...
void Fbx::RenderNMap(Transform& transform) {
	Direct3D::SetShader(SHADER_NMAP);
...
void Fbx::RenderAlpha(Transform& transform)
{
	for (int i = 0; i < materialCount_; i++)
	{
		//コンスタントバッファに情報を渡す
...
void Fbx::RenderWater(Transform& transform, float scroll) {
	Direct3D::SetShader(SHADER_WATER);
